...
void uLCD_4DLibrary::read_commande(uint8_t* cmd, uint16_t size)
{
	...
	{
		cpt = 0;
        while (!m_serial.readable() && cpt < 500000)
        {
          cpt ++;
        }
        if(cpt < 490000)
            cmd[i] = m_serial.getc();//m_serial.gets((char*)&cmd[i],1);
	}
}
...
uint16_t uLCD_4DLibrary::gfx_read_pixel(uint16_t center_x, uint16_t center_y)
{
    uint8_t cmd[6];
    cmd[0] = 0xFF;
    cmd[1] = 0xC0;
...
uint8_t uLCD_4DLibrary::gfx_move_origin(uint16_t x, uint16_t y)
{
    uint8_t cmd[6];
    cmd[0] = 0xFF;
    cmd[1] = 0xCC;
...
uint16_t uLCD_4DLibrary::file_mount()
{
	uint8_t cmd[3];
	cmd[0] = 0xFF;
	cmd[1] = 0x03;
...
uint8_t uLCD_4DLibrary::touch_detect_region(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    uint8_t cmd[10];
    cmd[0] = 0xFF;
    cmd[1] = 0x39;
...
uint8_t uLCD_4DLibrary::touch_set(uint8_t mode)
{
    uint8_t cmd[4];
    cmd[0] = 0xFF;
    cmd[1] = 0x38;
...
uint16_t uLCD_4DLibrary::touch_get(uint8_t mode)
{
    uint8_t cmd[4];
    cmd[0] = 0xFF;
    cmd[1] = 0x37;
